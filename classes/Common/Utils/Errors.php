...
class Errors
{
    ...
    public static function wpDie ( $message = '', $subtitle = '', $source = '', $exception = '', $title = '' )
    {
        if ( $message ) {
            ...
            Errors::writeLog( [ 'title' => $title . ' - ' . $subtitle, 'message' => $message, 'source' => $source, 'exception' => $exception ] );
            $source  = $source ? '<code>' . sprintf( __( 'Error source: %s', 'the-plugin-name-text-domain'  ), $source ) . '</code><BR><BR>' : '';
    ...
    public static function pluginDie ( $message = '', $subtitle = '', $source = '', $exception = '', $title = '' )
    {
        if ( $message ) {
            ...
            Errors::writeLog( [ 'title' => $title . ' - ' . $subtitle, 'message' => $message, 'source' => $source, 'exception' => $exception ] );
            $source = $source ? '<small>' . sprintf( __( 'Error source: %s', 'the-plugin-name-text-domain'  ), $source ) . '</small> - ' : '';
            ...
            global $_wp_this_plugin_die_notice;
            $_wp_this_plugin_die_notice = $error;
        ...
        add_action( 'admin_init',
            static function () {
                deactivate_plugins( plugin_basename( _THE_PLUGIN_NAME_PLUGIN_FILE ) );
            }
        );
    }
